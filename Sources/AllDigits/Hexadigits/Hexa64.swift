...
public struct ChiffreHexa: View {
    var value = 15
    var conf = Hexaconfig(.horiz)
    
    public init (_ val:Int, _ config:Hexaconfig) {
        value = val
        conf = config
    }
    
    var hexagram : Hexagramdata {
        Hexagramdata.byvalue(value)
    }
...
public struct NombreHexa: View {
    ...
    public init(_ value:Int, _ config:Hexaconfig,_ edit:Bool = false, _ base:Int = 64) {
        conf = config
        var tab : [Int] = []
        var mant = value
        while mant > 0 {
                    ...
                    VStack {
                        ...
                        Group{
                            if conf.style == .alter && i % 2 == 0 {
                                ChiffreHexa(chiffres[i], Hexaconfig(conf))
                            } else {
